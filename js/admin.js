...
(function () {
  ...
  if (fKategori) {
    fKategori.innerHTML = ONDER.CATEGORIES
  ...
  function editItem(id) {
    const it = ONDER.getAll().find(x => x.id === id);
    if (!it) return;
    fId.value = it.id;
  ...
  function renderTable() {
    ...
    tbody.innerHTML = list.map(it => `
      <tr>
        ...
        <td style="white-space:nowrap">${esc(ONDER.formatDate(it.tarih))}</td>
        <td>
  ...
  if (exportBtn) exportBtn.addEventListener("click", () => {
    const blob = new Blob([ONDER.exportJSON()], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "onder-mevzuat-" + new Date().toISOString().slice(0, 10) + ".json";
    a.click();
  ...
  if (importBtn && importInput) {
    ...
    importInput.addEventListener("change", () => {
      ...
      reader.onload = () => {
        try {
          ONDER.importJSON(reader.result);
