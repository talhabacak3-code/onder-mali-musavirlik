...
(function () {
    ...
    return `
      <article class="mev-item" data-cat="${esc(it.kategori || "Genel")}"
        ...
        <div class="mev-item-head">
          ...
          <span class="mev-date">${calIcon()} ${esc(ONDER.formatDate(it.tarih))}</span>
        </div>
        <h3>${esc(it.baslik)}</h3>
        <p>${esc(it.ozet || "")}</p>
        ${hasDetail ? `
  ...
  if (full) {
    ...
    if (filterBox) {
      const cats = ["Tümü"].concat(ONDER.CATEGORIES);
